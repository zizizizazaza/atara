...
export default function LoginModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-fade-in">
      ...
      />
      <div className="relative w-full max-w-[420px] rounded-3xl border border-core/10 bg-[#FBF6EE] shadow-[0_40px_100px_-24px_rgba(26,21,28,0.55)] p-8 md:p-9 animate-scale-in overflow-hidden">
        <div
          aria-hidden
          className="absolute -top-24 -right-20 w-[260px] h-[260px] rounded-full opacity-40 blur-3xl pointer-events-none"
          style={{ background: 'radial-gradient(circle at 50% 50%, #8CB6E8, transparent 65%)' }}
        />

        <button
          onClick={onClose}
          aria-label="close"
          className="absolute top-4 right-4 w-8 h-8 rounded-full text-core/50 hover:text-core hover:bg-core/8 transition-colors flex items-center justify-center z-10"
        >
        ...
        <div className="relative">
          <h2 className="font-display text-[1.6rem] leading-[1.1] font-semibold text-core tracking-tight">
            Log in to Atara
          </h2>
          <p className="mt-2 text-[0.85rem] text-core/60 leading-snug">
          ...
          <div className="my-6 flex items-center gap-3">
            ...
            <span className="font-mono text-[0.6rem] tracking-[0.22em] text-core/40">or email</span>
            <div className="h-px flex-1 bg-core/12" />
          </div>

          <label className="block">
            <div className="font-mono text-[0.66rem] text-core/55 mb-1.5">email</div>
            <div className="relative">
              <input
                type="email"
                value={email}
                ...
                className={`w-full bg-white border border-core/15 rounded-xl py-2.5 font-mono text-[0.85rem] text-core placeholder:text-core/30 focus:outline-none focus:border-glow transition-colors ${
          ...
          {lookupState === 'no-passkey' && (
            <label className="block mt-3 animate-fade-in">
              <div className="font-mono text-[0.66rem] text-core/55 mb-1.5">verify</div>
              <input
                type="text"
                inputMode="numeric"
                maxLength={6}
                ...
                placeholder="000000"
                className="w-full bg-white border border-core/15 rounded-xl px-3.5 py-2.5 font-mono text-[1rem] text-core tracking-[0.4em] tabular-nums text-center placeholder:text-core/20 focus:outline-none focus:border-glow"
              />
            </label>
          )}
          ...
          <button
            ...
            className="w-full mt-5 rounded-2xl bg-core text-peach py-3 px-5 flex items-center justify-center gap-2.5 hover:bg-core/90 disabled:bg-core/25 disabled:cursor-not-allowed transition-colors"
          >
            {busy === 'email' ? (
              <>
                <span className="w-4 h-4 border-2 border-peach/30 border-t-peach rounded-full animate-spin" />
                <span className="font-display font-semibold text-[0.9rem] tracking-tight">
          ...
          <div className="mt-6 pt-5 border-t border-core/8 font-mono text-[0.6rem] text-core/40 leading-relaxed text-center">
...
function ProviderButton({
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="group relative w-full flex items-center justify-center rounded-xl border border-core/12 bg-white px-4 py-2.5 text-[0.88rem] text-core font-medium hover:border-core/30 hover:shadow-[0_4px_14px_-6px_rgba(26,21,28,0.18)] disabled:opacity-50 disabled:cursor-not-allowed transition-all"
    >
      <span className="absolute left-4 w-5 h-5 flex items-center justify-center flex-shrink-0">
        {busy ? (
...
function TwitterIcon() {
  return (
    <svg viewBox="0 0 24 24" className="w-[16px] h-[16px]" fill="#1A151C">
...
function WalletIcon() {
  return (
    <svg viewBox="0 0 24 24" className="w-[18px] h-[18px]" fill="none" stroke="#1A151C" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
      ...
      <circle cx="16" cy="15" r="1" fill="#1A151C" stroke="none" />
    </svg>
  )
}
